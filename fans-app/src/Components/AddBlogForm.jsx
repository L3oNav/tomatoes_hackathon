...
import { ADD_BLOG } from '../queries/index'
...
class AddBlogForm extends React.Component {
    ...
    updateCache = (cache, { data: addBlog }) => {
        const { getSortedBlogs } = cache.readQuery({ query: GET_BLOGS_NAMES })
        cache.writeQuery({
            query: GET_BLOGS_NAMES,
            data: {
                getSortedBlogs: [addBlog, ...getSortedBlogs],
            },
        })
        window.location.reload()
    }
        ...
        return (
            ...
            >
                {(addBlog, { data, loading, error }) => {
                    ...
                    return (
                        <Container>
                            <Title>Añade un Blog</Title>
                            <Form onSubmit={(event) => this._handleSubmit(event, addBlog)}>
                                <Input
                                    type='text'
                                    name='name'
                                    placeholder='Nombre'
